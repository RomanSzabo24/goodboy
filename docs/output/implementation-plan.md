...
- [x] ⭐ SEO — **done**, including real per-step metadata (was 🟡 partial as of 2026-08-31; the assignment names this explicitly — "implement og:image and various titles and descriptions on different form steps" — so the client-side-only mitigation below wasn't treated as good enough):
  ...
  - 📍 `cc56e64 feat: add SEO og:image and metadataBase` — committed; the per-step routing work above is not yet committed (see Phase 10)
...
**Not yet committed as of 2026-08-31:** the per-step routing refactor described in Phase 9's SEO bullet (new `(donation)` route group/layout/provider/context, deleted `donation-form.tsx`/`donation-form-store.ts`, the `eslint.config.mjs` ignore fix, and the e2e selector fix) — `git status` still shows these as untracked/modified pending the repo owner's review.
