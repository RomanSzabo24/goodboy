...
- [x] Transitions/animations between steps — **not done.** Steps swap instantly with no animation. This is explicitly called out as rewarded in the assignment and is a clear follow-up (e.g. a simple fade/slide via `tw-animate-css`, which is already installed).
